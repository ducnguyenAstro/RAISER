...
trainpath = str(args.folderName) 	# default = 'train'
...
if args.qmatrix:
    try: 
        with open("q.p", "rb") as fp:
            Q = pickle.load(fp)
        print ("load the q file successfully.") 
    except:
        print ("cannot load the q file")

if args.vmatrix:
    try:
        with open("v.p", "rb") as fp:
            V = pickle.load(fp)
        print ("load the v file successfully.") 
    except:
        print ("cannot load the v file")
...
dirnames = os.path.dirname(trainpath)
...
# store filter files as zip file

if args.zip:
    zipFilter(dirnames,str(R),"q.p","v.p","filter.p")
